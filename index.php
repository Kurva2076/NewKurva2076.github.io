...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (!empty($_COOKIE['rerouteButton']) and strcmp($_COOKIE['rerouteButton'], 'welcome_page') != 0) {
        $page_name = (strcmp($_COOKIE['rerouteButton'], 'send') == 0 or
            strcmp($_COOKIE['rerouteButton'], 'sign_in') == 0 or
            strcmp($_COOKIE['rerouteButton'], 'Сохранить') == 0) ? 'form.php' : (
                (strcmp($_COOKIE['rerouteButton'], 'edit') == 0 or
                    strcmp($_COOKIE['rerouteButton'], 'sign_out') == 0) ? 'authorization.php' : (
                        (strcmp($_COOKIE['rerouteButton'], 'admin') == 0) ? 'admin.php' : ''
                )
        );
...
//        print_r($_SERVER);
    } elseif (strcmp($_POST['rerouteButton'], 'admin') == 0) {

        if (empty($_SERVER['PHP_AUTH_USER']) or
            empty($_SERVER['PHP_AUTH_PW']) or
            $_SERVER['PHP_AUTH_USER'] != 'admin' or
        ...
        $error_messages = [
            'login' => [
                400 => 'Введите логин.',
                4101 => 'Пользователя с таким логином не найдено.',
                4102 => 'Администратора с таким логином не найдено.'
            ],
            'password' => [
                400 => 'Введите пароль.',
        ...
        if (empty($_POST['login'])) {
            ...
            if (empty($user_info))
                $system_message = $error_messages['login'][4101];
        ...
        if (empty($_SESSION['user_id'])) {
            ...
            try {
                ...
                $users_stmt = $db->prepare(
                    ...
                    "agreement = :agreement WHERE user_id = :user_id;"
                );
                $users_stmt->bindParam(':full_name', $_POST['fullName']);
                $users_stmt->bindParam(':birth', $datebirth);
                $users_stmt->bindParam(':sex', $_POST['sex']);
